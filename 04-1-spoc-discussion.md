...
1. 寄存器、高速缓存、内存、外存的访问特征？
 ...
 > 访问速度：寄存器 > 高速缓存 > 内存 > 外存；大小：寄存器 < 高速缓存 < 内存 < 外存。

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
 
 > “均衡繁忙”状态是指各层存储介质均处于繁忙状态，但是不存在某一种介质成为性能瓶颈；
...
1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
 ...
 > 覆盖：将程序划分成模块，不会同时执行的模块可以使用同一块内存，即将执行的模块会将已经执行完成的模块从内存中替换出去。程序开发者主要工作是划分模块和确定模块之间执行的依赖关系。
...
2. 什么是交换技术？覆盖与交换有什么不同？
 ...
 > 交换：以进程为单位的内存替换。覆盖通过划分模块，减小单个程序运行占用的内存；而交换通过交换进程，解决内存空间对于进程不足的问题。
...
> 时间局部性：当前执行的指令或访问的数据在短时间内很可能再次被访问到。
> 空间局部性：当前执行的指令或访问的数据附近的指令或数据有大概率被访问到。
> 分支局部性：分支跳转的某一分支，有可能会被多次访问。
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
 ...
 > 虚拟存储是指只将当前指令执行需要的部分页面或段装入内存的内存管理方式，在有限容量的内存中，以页为单位自动装入更多更大的程序。好处：支持内存的不连续性、大用户空间、部分交换等;挑战：增加内存管理的难度。
 
2. 虚拟存储需要什么样的支持技术？

 > 硬件上需要MMU，完成地址转换等，操作系统内核需要实现置换算法。
...
### 8.5 虚拟页式存储
 ...
 2. 页式存储和虚拟页式存储的区别是什么？
 > 虚拟页式存储是指在页式存储管理的基础上，增加请求调页和页面置换的功能。
### 8.6 缺页异常

1. 缺页异常的处理流程？


 > 页表项引用；页面不存在，触发缺页异常；查找在外存中的页面；页面换入；页表项修改；重新执行导致缺页异常的指令。

2. 虚拟页式存储管理中有效存储访问时间是如何计算的？

 > EAT = EAT = 访存时间 * (1-p) + 缺页异常处理时间 * 缺页率p
