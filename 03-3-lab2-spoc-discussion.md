...
* 访问门（中断响应、函数调用）时，要求CPL<=DPL[门]且CPL>=DPL[段]，访问段时（指令执行），要求CPL<=DPL且RPL<=DPL。
...
* 指令在访问数据段、访问页、进入中断服务例程、执行特权指令时会有特权级判断
...
* 构造符合int、iret实现的堆栈，调用int、iret指令。

5. TSS和Task Register的作用是什么？
* TSS用于任务切换时保存寄存器等信息，Task Register中包括段选择子、基地址、段大小限制等信息，用来确定TSS的位置。
...
* （段页式）逻辑地址输入MMU，被分为段选择子和偏移，MMU在GDT中查找段的基地址，加上偏移后构成线性地址；线性地址被分为三段，通过两级寻址确定页基址，再通过页内偏移确定访问的实际物理地址。
